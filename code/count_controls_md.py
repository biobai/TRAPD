...
def extractcounts(pops, vcfline, max_ac, max_af, popmax_af,min_an):
	...
	an_out=100000000000
	af_popmax_out=0
	if ";controls_ac_nfe=" in (";"+vcfline) and ";controls_an_nfe=" in (";"+vcfline):
		ac_out=num_convert((";"+vcfline).split((";controls_ac_nfe="))[1].split(";")[0].split(",")[0],0)
		an=num_convert((";"+vcfline).split((";controls_an_nfe="))[1].split(";")[0].split(",")[0], 100000000000)
	if ac_out==0:
		af_out=0
	else:
		if ";controls_af_nfe=" in vcfline:
			af_out=num_convert((";"+vcfline).split((";controls_af_nfe="))[1].split(";")[0].split(",")[0],0)
		else:
	...
	if (ac_out>float(max_ac)) or (af_out>float(max_af)) or (an<float(min_an)) or (af_popmax_out>float(popmax_af)):
		...
		if options.database=="gnomad":
			if "ALL" in pops:
				if ";controls_nhomalt_nfe=" in (";"+vcfline):
					hom_out=(";"+vcfline).split((";controls_nhomalt_nfe="))[1].split(";")[0].split(",")[0]
				elif ";hom=" in (";"+vcfline):
